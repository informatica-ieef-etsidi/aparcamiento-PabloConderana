...
void recuento(struct plaza aparcamiento[NPLAZAS], int dim, int *nLibresCoches, int *nLibresMotos);
void inicializar(struct plaza aparcamiento[NPLAZAS], int dim);
int buscarPlazaLibre(struct plaza aparcamiento[NPLAZAS], int dim, char tipo);
int buscarVehiculo(struct plaza aparcamiento[NPLAZAS], int dim, char matricula[8]);
...
void main() {
	...
	int nplazascochel, nplazasmotol;
	char tipocoche = 'C', tipomoto = 'M';
	...
	while (1) {
		...
		case 'r':
			...
			case 'c':
				...
				ocupamiento = buscarPlazaLibre(aparcamiento, NPLAZAS, tipocoche);
			...
			case 'M':
			case 'm':
				ocupamiento = buscarPlazaLibre(aparcamiento, NPLAZAS, tipomoto);
		...
		case 'E':
		case 'e':
			recuento(aparcamiento, NPLAZAS, &nplazascochel, &nplazasmotol);

			printf("Hay %d plazas de coche libres y %d plazas de moto libres.\n\n", nplazascochel, nplazasmotol);
		...
		case 'b':
			...
			aux = buscarVehiculo(aparcamiento, NPLAZAS, matricula_comparar);
...
void recuento(struct plaza aparcamiento[NPLAZAS], int dim, int *nLibresCoches, int *nLibresMotos) {
	...
	for (i = 0, *nLibresCoches = 0, *nLibresMotos = 0; i < dim; i++) {
		if (aparcamiento[i].estado == 0 && aparcamiento[i].tipo == 'C') {
			*nLibresCoches += 1;
		}
...
int buscarPlazaLibre(struct plaza aparcamiento[NPLAZAS], int dim, char tipo) {
	...
	for (i = 0, plazaslibres = 1; i < dim; i++) {
...
int buscarVehiculo(struct plaza aparcamiento[NPLAZAS], int dim, char matricula[8]) {
	...
	case 'c':
	case 'C':
		for (i = 0, vehiculoencontrado = 1; i < dim - 5; i++) {
			if ((strcmp(matricula, aparcamiento[i].matricula)) == 0) {
				printf("El coche de matricula %s se encuentra estacionado en la plaza %d.\n\n", matricula, i + 1);
	...
	case 'm':
	case 'M':
		for (i = 5, vehiculoencontrado = 1; i < dim; i++) {
			if ((strcmp(matricula, aparcamiento[i].matricula)) == 0) {
				printf("El coche de matricula %s se encuentra estacionado en la plaza %d.\n\n", matricula, i + 1);
				vehiculoencontrado = 0;
				break;
